...
#     def __init__(self, cls_ary: List[str], device: Union[str, torch.device] = 'cpu'):
#         self.device = device
...
#         self.txt_features = self.get_txt_features(self.classes)
#
...
#         txt_features = self.txt_features
#
...
def get_cached_datafile(filename: str):
    return hf_hub_download(
        repo_id=HF_DATAFILE_REPO, filename=filename, repo_type=HF_DATAFILE_REPO_TYPE
    )
...
class BaseClassifier(object):
    def __init__(
        self,
        model_str: str = BIOCLIP_MODEL_STR,
        pretrained_str: Union[str, None] = None,
    ...
    def load_pretrained_model(
        ...
        if pretrained_str is None and len(pretrained_tags) > 0:
            if len(pretrained_tags) > 1:
                raise ValueError(
                    f"Multiple pretrained tags available {pretrained_tags}, must provide one"
                )
        ...
        self.preprocess = (
            preprocess_img if self.model_str == BIOCLIP_MODEL_STR else preprocess
        )
    ...
    @torch.no_grad()
    def create_image_features_for_image(
        self, image: str | PIL.Image.Image, normalize: bool
    ) -> torch.Tensor:
    ...
    def create_probabilities(
        self, img_features: torch.Tensor, txt_features: torch.Tensor
    ) -> dict[str, torch.Tensor]:
        logits = self.model.logit_scale.exp() * img_features @ txt_features
        return F.softmax(logits, dim=1)

    def create_probabilities_for_images(
        self, images: List[str] | List[PIL.Image.Image], txt_features: torch.Tensor
    ) -> dict[str, torch.Tensor]:
...
class CustomLabelsClassifier(BaseClassifier):
    ...
    @torch.no_grad()
    def predict(
        self, images: List[str] | str | List[PIL.Image.Image], k: int = None
    ) -> dict[str, float]:
    ...
    def group_probs(
        self, image_key: str, img_probs: torch.Tensor, k: int = None
    ) -> List[dict[str, float]]:
        result = []
...
def predict_classifications_from_list(
...
class TreeOfLifeClassifier(BaseClassifier):
    ...
    def format_species_probs(
        self, image_key: str, probs: torch.Tensor, k: int = 5
    ) -> List[dict[str, float]]:
        topk = probs.topk(k)
        result = []
        for i, prob in zip(topk.indices, topk.values):
    ...
    def predict(
        self,
        images: List[str] | str | List[PIL.Image.Image],
        rank: Rank,
        min_prob: float = 1e-9,
        k: int = 5,
        ...
        for i, image in enumerate(images):
            ...
            if rank == Rank.SPECIES:
                ...
                result.extend(
                    self.format_grouped_probs(key, probs[key], rank, min_prob, k)
                )
        return result
