...
#     parser.add_argument('model_checkpoint_path', type=str, help='The full path to the model checkpoint')
#     parser.add_argument('out_csv_path', type=str, help='The full path to the output csv file')
#     parser.add_argument('gt_csv_path', type=str, nargs='?', default=None, help='The full path to the ground truth csv (optional)')
...
#     original_csv['bbox'] = list(zip(original_csv['bbox x'], original_csv['bbox y'], original_csv['bbox w'], original_csv['bbox h']))
#
#     # Split the original dataframe into two based on the filtering criteria
#     filtered_test = original_csv[(original_csv[['bbox x', 'bbox y', 'bbox w', 'bbox h']].notna().all(axis=1)) & (original_csv['annot species'] == config['species'])].reset_index(drop = True)
#     other_test = original_csv[~(original_csv[['bbox x', 'bbox y', 'bbox w', 'bbox h']].notna().all(axis=1)) | (original_csv['annot species'] != config['species'])].reset_index(drop = True)
